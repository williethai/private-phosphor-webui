    ...
    angular
        ...
        .factory('APIUtils', ['$http', 'Constants', '$q', 'dataService',function($http, Constants, $q, DataService){
          ...
          var SERVICE = {
			  ...
			  fanControl: function(callback, fan, speed){
				  if (fan === 0){
                $http({
                  method: 'PUT',
                  url: DataService.getHost() + "/xyz/openbmc_project/sensors/fan_tach/psu0_fan/attr/Target",
                  headers: {
                      'Accept': 'application/json',
                      'Content-Type': 'application/json'
                  },
                  withCredentials: true,
                  data: JSON.stringify({"data": speed})
                }).then(function(response){
                      var json = JSON.stringify(response.data);
                      var content = JSON.parse(json);
                      if(callback){
			  ...
			  if (fan === 1){
				$http({
                  method: 'PUT',
                  url: DataService.getHost() + "/xyz/openbmc_project/sensors/fan_tach/psu1_fan/attr/Target",
                  headers: {
                      'Accept': 'application/json',
                      'Content-Type': 'application/json'
                  },
                      ...
                      for(var key in content.data){
                        if(content.data.hasOwnProperty(key) && content.data[key].hasOwnProperty('Unit')){
                          ...
                          sensorData.push(Object.assign({
                            ...
                            confirm: false,
                            copied: false,
                            sensor_type: key.substring(key.search("/sensors/") + "/sensors/".length, key.lastIndexOf("/")),
                            title: title,
              ...
              getSingleSensorStatus: function(callback, sensor){
                $http({
                  method: 'GET',
                  url: DataService.getHost() + sensor.path,
                  headers: {
                      'Accept': 'application/json',
                      'Content-Type': 'application/json'
                  },
                  withCredentials: true
                      ...
                      if(content.data){
                        if(content.data.hasOwnProperty('Unit')){

                          severity = getSensorStatus(content.data);

                          if(!content.data.hasOwnProperty('CriticalLow')){
                            content.data.CriticalLow = "--";
                            content.data.CriticalHigh = "--";
                          }

                          if(!content.data.hasOwnProperty('WarningLow')){
                            content.data.WarningLow = "--";
                            content.data.WarningHigh = "--";
                          }
							/*
                          tempKeyParts = key.split("/");
                          title = tempKeyParts.pop();
						...
						*/
                          content.data.Value = getScaledValue(content.data.Value, content.data.Scale);
                          content.data.CriticalLow = getScaledValue(content.data.CriticalLow, content.data.Scale);
                          content.data.CriticalHigh = getScaledValue(content.data.CriticalHigh, content.data.Scale);
                          content.data.WarningLow = getScaledValue(content.data.WarningLow, content.data.Scale);
                          content.data.WarningHigh = getScaledValue(content.data.WarningHigh, content.data.Scale);
                          if(Constants.SENSOR_SORT_ORDER.indexOf(content.data.Unit) > -1){
                            customOrder = Constants.SENSOR_SORT_ORDER.indexOf(content.data.Unit);
                          }else{
                            customOrder = Constants.SENSOR_SORT_ORDER_DEFAULT;
                          }
						  function deepCopy(desObj, oldObj) {
							    if (oldObj && typeof oldObj === 'object') {
							        //desObj = Object.prototype.toString.call(oldObj) === "[object Array]" ? [] : {};
							        for (var i in oldObj) {
							            desObj[i] = deepCopy(desObj[i], oldObj[i]);
							        }
							    }
								return oldObj;
							}
						  console.log("old sensor");
                          var _sensor = Object.assign({
                            path: sensor.path,
                            selected: false,
                            confirm: false,
                            copied: false,
                            ...
                            title: sensor.title,
                            unit: Constants.SENSOR_UNIT_MAP[content.data.Unit],
                            severity_flags: severity.flags,
                            status: severity.severityText,
                            order: severity.order,
                            custom_order: customOrder,
                            search_text: (title + " " + content.data.Value + " " +
                               Constants.SENSOR_UNIT_MAP[content.data.Unit] + " " +
                               severity.severityText + " " +
                               content.data.CriticalLow + " " +
                               content.data.CriticalHigh + " " +
                               content.data.WarningLow + " " +
                            ...
                            original_data: {key: sensor.path, value: content.data}
                          }, content.data);
						  deepCopy(sensor, _sensor);
                        }
                      }
              ...
              setSensorAttr: function(callback, key, attr, value){
              		console.log("setSensorAttr: "  + key + " " + attr + " " + value);
	                $http({
	                  method: 'PUT',
	                  url: DataService.getHost() + key + "/attr/" + attr,
	                  headers: {
	                      'Accept': 'application/json',
	                      'Content-Type': 'application/json'
	                  },
              ...
              setSensorAttr2: function _setSensorAttr2(callback, _sensor){
              		if(_sensor.length == 0)return callback(0);
	                $http({
	                  method: 'PUT',
	                  url: DataService.getHost() + _sensor[0].key + "/attr/" + _sensor[0].attr,
	                  headers: {
	                      'Accept': 'application/json',
	                      'Content-Type': 'application/json'
	                  },
	                  withCredentials: true,
	                  data: JSON.stringify({"data": _sensor[0].value})
	                }).then(function(response){
	                      var json = JSON.stringify(response.data);
	                      var content = JSON.parse(json);
						  ...
						  _sensor.splice(0, 1);
						  console.log(_sensor);
						  _setSensorAttr2(callback, _sensor);
	                      //if(callback){
	                      //    return callback(content.status);
	                      //}
	                }, function(error){
